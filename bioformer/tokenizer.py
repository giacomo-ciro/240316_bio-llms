...
class Tokenizer(nn.Module):
    ...
    def tokenize_and_pad_batch(
        ...
        tokenized_data = self.tokenize_batch(data,
                                             gene_ids,
                                             )

        batch_padded = self.pad_batch(tokenized_data,
                                      max_len,
                                      )
        return batch_padded
    ...
    def random_mask_value(
        ...
        if isinstance(values, torch.Tensor):
            values = values.clone().detach().numpy()    # it is crutial to clone the tensor, otherwise it changes the original tensor
        else:
            values = values.copy()
